...
├── .gitignore                     # flutter create's own + appended socle/secrets (delivered last batch) — @rules/config.md
├── README.md
├── docs/
...
| 3     | `main.dart` + `pubspec.yaml` + `analysis_options.yaml` + `.gitignore` (appended to flutter create's) + README + instructions |
...
| 4     | `main.dart` + `pubspec.yaml` + `analysis_options.yaml` + `.gitignore` (appended to flutter create's) + README + instructions |
...
- Last batch: instructions (`flutter pub get`, `dart run build_runner build --delete-conflicting-outputs`, then the install method chosen in Phase 1 — USB direct by default, release build + keystore only if a Signed release APK / Play Store AAB was selected; see `@rules/config.md §Installation methods`) + `.gitignore` **appended** to the one `flutter create` generated (`@rules/config.md §.gitignore`) + `README.md` at the root.
...
Delivered **inside the last code batch** of generation (the batch already carrying `main.dart` + root configs — no dedicated seed batch, the announced batch total stays the calibration count) as a standalone script `tool/seed.dart` (run with `dart run tool/seed.dart`):
