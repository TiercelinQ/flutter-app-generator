...
flutter test                                          # ONLY if tests enabled in Phase 1 / test/ exists — otherwise skip
flutter build apk                                     # compile smoke (debug) — last batch; use --release / `appbundle` only if a Signed release APK / Play Store AAB was selected (Phase 1)
```

Rules:
- A non-zero exit or any reported issue is a failure → fix the root cause, do not silence the rule. Re-run until clean.
- **`flutter test`**: run only if tests were enabled in Phase 1 / `test/` contains tests. **Do not create tests if the project has none** — only run what exists.
...
6. Design-system compliance — depends on the mode (`docs/specs/01-scoping.md` / `04-architect.md`). `framework`: `design-system.md` + the composition validated in `docs/specs/03-surfaces.md`/`04-architect.md` + the retained `layout.md` specs (toasts, dialogs), zero hardcoded visual value (tokens / `context.colors`), depth by stroke (radius via `AppTokens.radius` only, `elevation: 0`, no `BoxShadow`). `native`: `@rules/native-design.md` — colors from `Theme.of(context).colorScheme` (no raw hex except `seedColor`), spacing/sizes from `AppTokens`, Material components; the stroke-based skin checks do not apply. See `@rules/theme.md` / `@rules/native-design.md`.
