...
- **In-contract** — a new entity or an extension within the layer split (`model + repository + controller + screen/widgets`), a new `@riverpod` provider (codegen), a schema change with its migration, a new component within the mode's tokens. Proceed to the diff as a straightforward addition.
- **Deviation** — a new library/dependency, a change of design-system mode, a second file for one entity beyond `model + repository + controller + screen/widgets`, a new Android permission, or anything the locked contract does not cover. **STOP → declare the deviation in the diff, explain why → wait for validation before writing.** Never exceed the contract silently. **The diff + validation IS the protocol.**
