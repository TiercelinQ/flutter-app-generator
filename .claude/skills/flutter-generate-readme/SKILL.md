...
[Then the 4 install methods, USB direct first — see @rules/config.md §Installation methods.
 Highlight the one chosen in Phase 1. Signing/keystore steps only for Signed release APK / Play Store AAB.
 Uninstall: clean removal (app + private data) — both cases documented (dev/sideload A/B/C vs Play Store D); highlight the one matching the Phase 1 choice.]
