...
> `S0` is reserved for the delivery baseline written automatically at the end of Phase 5 (`/flutter-p5-development`). Manual saves therefore start at `S1` — the `max + 1` rule above already yields `1` when only `S0` exists.
