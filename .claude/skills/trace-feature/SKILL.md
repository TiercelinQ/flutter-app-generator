...
# /trace-feature — Analyze a generated Flutter app
...
- **Do not** modify code during an analysis. This skill is read-only; if a fix is wanted, hand off to `/fix-issue` or `/add-feature`.
...
- **"And fix it"** → switch to `/fix-issue` (bug) or `/add-feature` (change), carrying the analysis findings.
- **"Is this the right architecture?"** → compare against `docs/specs/04-architect.md` and `rules/architecture.md`; report deviations, do not refactor (that is `/refactor-code`, on request).
