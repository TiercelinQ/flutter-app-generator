...
4. **Theme mapping**: `framework` — tokens → theme table (`app_theme.dart` elements and the tokens they consume). `native` — short `ColorScheme.fromSeed(seedColor)` mapping (seed → light/dark `ColorScheme`, `useMaterial3: true`) + the `AppTokens` retained for spacing/sizes/durations.
