...
- Write to the project root chosen at the start of the flow (via `/flutter-app` or `/p1-scoping`); if it was not set in this flow, ask for it once.
...
After resolving an anomaly: cleanup report (`rules/architecture.md`) then offer `Veux-tu mémoriser ce point ? /save-memory`.
