...
## Goal
Define the concrete screen layout (destinations, secondary panel, recurring components), starting from the `layout.md` proposed default, freely amendable by the user.
...
Based on the features validated in Phase 2 (`docs/specs/02-featuring.md`), propose the `layout.md` default composition adapted and justified against the features, and state explicitly that the user may amend any part of it or define a different composition. Produce (in the user's language):
...
Produce the complete synthesis of the validated layout (in the user's language): shell, destinations, panel, forms, gestures, toasts, the retained composition (default, amended, or user-defined) vs the `layout.md` proposed default.
